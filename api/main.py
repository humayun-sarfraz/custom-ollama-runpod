...
async def lifespan(app: FastAPI):
    """Manage shared HTTP client and check Ollama connectivity on startup."""
    app.state.http_client = httpx.AsyncClient(timeout=120)
    try:
        resp = await app.state.http_client.get(f"{OLLAMA_BASE_URL}/api/tags", timeout=5)
...
class ChatRequest(BaseModel):
    ...
    model: str = Field(DEFAULT_MODEL, max_length=100, pattern=r"^[a-zA-Z0-9._:-]+$")
    stream: bool = False
...
<body>
  ...
  <div class="config">
    <div><label>Model</label><br><input id="model" value="{DEFAULT_MODEL}"></div>
